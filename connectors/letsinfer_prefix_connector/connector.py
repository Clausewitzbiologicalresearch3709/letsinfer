...
"""Let's Infer generic prefix cache as a vLLM v1 KV connector (M1 v1).

Out-of-tree connector for the pinned official image (vLLM 0.26.0):
...
- capture happens at INTERMEDIATE chunked-prefill boundaries P
  (P = num_computed + num_scheduled at the end of a prefill step with
  P < prompt_len): the pages then hold exactly KV[0..P) and SSM@P;
...
Known v1 limits (documented, revisit by measurement): capture is
...
class LetsInferPrefixConnector(KVConnectorBase_V1, SupportsHMA):
    ...
    def __init__(
        ...
        logger.info(
            ...
            "attn_block_size=%d, min_tokens=%d, fingerprint=%s",
            role.name,
            len(self._groups),
    ...
    def build_connector_meta(
        ...
        for req_id, scheduled in scheduler_output.num_scheduled_tokens.items():
            ...
            if (
                ...
                and (exact_boundary or position < len(prompt))
